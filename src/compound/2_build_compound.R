...
# TODO - only delete the outdated parquet files, right now dvc doesn't persist
pacman::p_load(future)

workers <- future::availableWorkers() |> length()
future::plan(future::multicore(workers=workers))

outdir <- fs::dir_create("brick/compound_sdf.parquet")

write_sdfparquet <- function(file,outdir,nmax=5e7){
  ...
  while(length(buff) != 0){
    ...
    arrow::write_parquet(proptbl, iname)
  }
...
gzfiles <- fs::dir_ls("download/compound/sdf", glob="*sdf.gz$")
start <- Sys.time()

# Takes about 2 hours
futures <- gzfiles |> purrr::map(\(gzfile){
  print(Sys.time()-start)
  future::future({write_sdfparquet(gzfile,outdir)})
},.progress=TRUE)
